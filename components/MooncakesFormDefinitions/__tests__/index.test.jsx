...
describe('Mooncakes Form Definitions', () => {
  ...
  it('renders new definition', async () => {
    render(<MooncakesFormDefinitions word={databaseDataMock[0]}/>)
    const addButtonTrigger = screen.getByRole('button', { name: 'Add Definition'})
  ...
  it('types in a new definition', async () => {
    ...
    const definitions = screen.getAllByRole('textbox');
    await userEvent.type(definitions[definitions.length - 1], 'last definition');
  ...
  it('types in two new definitions', async () => {
    ...
    const definitions = screen.getAllByRole('textbox');
    await userEvent.type(definitions[definitions.length - 1], 'last definition');
    await userEvent.type(definitions[definitions.length - 2], 'second to last definition');

    expect(definitions[definitions.length - 2]).toHaveValue('second to last definition');
  })
