...
describe("Mooncakes Form Sentences", () => {
  ...
  it("renders first sentence", () => {
    render(<MooncakesFormSentences word={databaseDataMock[0]} />);
    const firstSentenceKr =
      screen.getByDisplayValue("그녀는 그에게 사랑을 고백했다.");
    const firstSentenceEn = screen.getByDisplayValue(
  ...
  it("renders new empty sentence then specifically deletes sentence", async () => {
    ...
    const validValues = [
      "그녀는 그에게 사랑을 고백했다.",
      "사랑은 인생의 가장 강력한 감정 중 하나입니다.",
      "그들은 서로 사랑하는 마음을 키워나갔다.",
    ];
  ...
  it("types in a new sentence", async () => {
    ...
    await userEvent.type(
      ...
      "행복은 종종 작은 것에서 찾을 수 있습니다.",
    );

    expect(sentences[sentences.length - 1]).toHaveValue(
      "행복은 종종 작은 것에서 찾을 수 있습니다.",
    );
  });
  ...
  it("types in two new sentences, updates first and last", async () => {
    ...
    const validValues = [
      "그녀는 그에게 사랑을 고백했다.행복은 종종 작은 것에서 찾을 수 있습니다.",
      "사랑은 인생의 가장 강력한 감정 중 하나입니다.",
      "그들은 서로 사랑하는 마음을 키워나갔다.",
      "행복은 종종.",
    ];
  ...
  it("types in two new sentences then deletes second to last one", async () => {
    render(<MooncakesFormSentences word={databaseDataMock[0]} />);
    const addButtonTrigger = screen.getByRole("button", {
      name: "Add Sentence",
    });
    ...
    await userEvent.type(sentences[sentences.length - 2], "행복은 종종.");

    const deleteButtons = screen.getAllByLabelText("delete icon");

    // all but first sentence
    await userEvent.click(deleteButtons[deleteButtons.length - 2]);
    const updatedSentences = screen.getAllByLabelText("Korean");
