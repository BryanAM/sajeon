...
describe('Sajeon Pagination Basics', () => {
  it('renders the component', () => {
    const results = render(<SajeonPagination pages={5} currentPage={{ page: 1 }} />)
  ...
  it('Expect previous to be disabed on load', () => {
    ...
    const previousLink = results.getByRole('link', { name: 'Go to previous page'})
...
describe('Sajeon Pagination Ellipsis Testing', ()=> {
  it('One page, no ellipsis', () => {
    const results = render(<SajeonPagination pages={1} currentPage={{ page: 1 }} />)

    const span = results.queryByText('More pages', { hidden: true });
    expect(span).not.toBeInTheDocument();
  })

  it('Two pages, no ellipsis', () => {
    const results = render(<SajeonPagination pages={2} currentPage={{ page: 1 }} />)

    const span = results.queryByText('More pages', { hidden: true });
    expect(span).not.toBeInTheDocument();
  })

  it('Three pages, no ellipsis', () => {
    const results = render(<SajeonPagination pages={3} currentPage={{ page: 1 }} />)

    const span = results.queryByText('More pages', { hidden: true });
    expect(span).not.toBeInTheDocument();
  })

  it('Four pages, ellipsis found', () => {
    const results = render(<SajeonPagination pages={4} currentPage={{ page: 1 }} />)
  ...
  it('Last page, ellipsis found i.e. because there are previous ones', () => {
    const results = render(<SajeonPagination pages={7} currentPage={{ page: 7 }} />)
