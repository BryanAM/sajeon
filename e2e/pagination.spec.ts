...
test('Should advance to next page when clicking Next pagination button', async ({ page }) => {
  await page.goto('http://localhost:3000');
  await page.getByRole('searchbox').fill('study');
  await page.getByRole('button', { name: 'Search' }).click();
  await expect(page).toHaveURL('http://localhost:3000/search/study')
  ...
  await expect(page).toHaveURL('http://localhost:3000/search/study?page=2')
...
test('Should advance twice when clicking Next pagination button', async ({ page }) => {
  await page.goto('http://localhost:3000');
  await page.getByRole('searchbox').fill('study');
  await page.getByRole('button', { name: 'Search' }).click();
  await expect(page).toHaveURL('http://localhost:3000/search/study')
  ...
  await expect(page).toHaveURL('http://localhost:3000/search/study?page=2')
  await page.getByRole('link', { name: 'Go to next page' }).click();
  await expect(page).toHaveURL('http://localhost:3000/search/study?page=3')
...
test('Should advance directly to a specific page if we enter a unique URL', async ({ page }) => {
  await page.goto('http://localhost:3000');
  await page.getByRole('searchbox').fill('study');
  await page.getByRole('button', { name: 'Search' }).click();
  await expect(page).toHaveURL('http://localhost:3000/search/study')

  await page.goto('http://localhost:3000/search/study?page=7');
...
test('Should go back if we click the previous button N times', async ({ page }) => {
  await page.goto('http://localhost:3000');
  await page.getByRole('searchbox').fill('study');
  await page.getByRole('button', { name: 'Search' }).click();
  await expect(page).toHaveURL('http://localhost:3000/search/study')
