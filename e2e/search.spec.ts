...
test("should do nothing when search is empty and we attempt to search an item", async ({
  page,
}) => {
...
test("should properly update search endpoint when submit fires study", async ({
  page,
}) => {
  await page.goto("http://localhost:8080");
  ...
  await expect(page).toHaveTitle(
    'Korean Dictionary Search Results For "study"',
  );
});

test("should properly update search endpoint when submit fires 공부하다", async ({
  page,
}) => {
  await page.goto("http://localhost:8080");
  ...
  await expect(page).toHaveTitle(
    'Korean Dictionary Search Results For "공부하다"',
  );
});

test("should load input when link is shared with endpoint 공부하다", async ({
  page,
}) => {
...
test('should load new input after link is shared with endpoint 공부하다 and searched to new endpoint "study"', async ({
  page,
}) => {
  await page.goto("http://localhost:8080");
  ...
  await Promise.all([
    page.waitForURL("http://localhost:8080/search/test"),
